...
let drag_lists = document.querySelectorAll('.drag_list');
let token = document.querySelector('meta[name="csrfToken"]').getAttribute('content');

if( drag_lists.length > 0 ){
	for(let i = 0; i < drag_lists.length; i++){
		drag_lists[i].addEventListener('dragover', e => {
		    e.preventDefault();
		    let box = drag_lists[i];
	...
	let drag_items = $('.drag_item');
	if( drag_items.length > 0 ){
		for( let j = 0; j < drag_items.length; j++  ){
			
			$(drag_items[j]).on("dragstart", function(){
				$(drag_items[j]).addClass('dragging');
			});

			$(drag_items[j]).on("dragend", function(){
				$(drag_items[j]).removeClass('dragging');

				let box_parent = $(drag_items[j]).parent('.drag_list').parent('.table').parent('.drag_box');
		        // let token = $('.drag_item').eq(0).find('input[name="_csrfToken"]').val();
		        let sorting_url = $(drag_items[j]).parent('.drag_list').attr('data-href');
		        let changed_list = {};
		        let this_items = this.parentNode.querySelectorAll('.drag_item');
		        let first = $(drag_items[j]).parent('.drag_list').attr('data-sort-begin');

				$(box_parent).addClass('preload')
		        this_items.forEach((n, i) => {
		            if( n.dataset.priority != (i + Number(first)) ){
		                n.dataset.priority = i + Number(first);
		                $(n).find('.sort_num').text(i + Number(first));
		                changed_list[n.dataset.id] = Number(n.dataset.priority);
		            }
		        });
		        ...
		        $.ajax({
		        	...
		        	console.log('done');
		        	console.log(data);
		        	$(box_parent).removeClass('preload')
		        }).fail(function(data){
		        	console.log('fail');
		        	console.log(data);
		        	$(box_parent).removeClass('preload')
...
function getDragAfterElement(box, y) {
    const draggableElements = [...box.querySelectorAll('.drag_item:not(.dragging)')];
