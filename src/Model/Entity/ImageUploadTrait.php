...
trait ImageUploadTrait{
            ...
            if(is_string($file)){
                ...
                switch($mimeType){
                    ...
                    case("image/webp"): // Добавляем поддержку WebP
                        return $this->_customUploadImg($file, $folder, $max_w, $max_h, $th_w, $th_h);
                    default:
                        $msg = 'Error: Unsupported mimetype; Folder: ' . $folder . ' fileinfo: ' . json_encode($_FILES);
                        $this->_setLogMsg($msg, 'img_uploads');
                        return false;
                }
            }
        } catch (\Exception $e) {
            $this->_setLogMsg('Error in uploadImageFunc: ' . $e->getMessage() . ' Folder: ' . $folder . ' Fileinfo: ' . json_encode($_FILES), 'img_uploads');
            return false;
        }
    }

    private function getLogMessage($file, $folder) {
        if (is_string($file)) {
            ...
            $size = 0;
            return 'Folder: ' . $folder . ' File: ' . $filename . ' Type: ' . $type . ' Size: ' . $size;
        }
    }
    ...
    protected function _customUploadImg($file = array(), $folder, $max_w, $max_h, $th_w, $th_h){
        try {
            ...
            $this->_setLogMsg('Error in _customUploadImg: ' . $e->getMessage() . ' Folder: ' . $folder . ' Fileinfo: ' . json_encode($_FILES), 'img_uploads');
            return false;
        }
    }
    ...
    protected function _resizeImg($target, $dest, $wmax = 300, $hmax = 300, $ext){
        ...
        */
        try {
            ...
            $this->_setLogMsg('Error in _resizeImg: ' . $e->getMessage() . ' Folder: ' . $folder . ' Fileinfo: ' . json_encode($_FILES), 'img_uploads');
            return false;
        }
    ...
    //        $this->_resizeImg($path_original, $path_th, $th_w, $th_h, $ext);
            ...
            $this->_setLogMsg('Error in _baseImgDecode: ' . $e->getMessage() . ' Folder: ' . $folder . ' Fileinfo: ' . json_encode($_FILES), 'img_uploads');
            return false;
        }
    }
